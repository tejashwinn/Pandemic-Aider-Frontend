...
import pandemic.aider.server.dao.DataAccess;
...
//				obj.addToPosts(postDetails);
	...
	private boolean checkForAllPosts(String username) {
		
		String sqlQuery = "Select * FROM posts WHERE P_UID= " + "'" + username + "'";
		try {
			Statement statement = connection.createStatement();
			ResultSet result = statement.executeQuery(sqlQuery);
	...
	public boolean deleteALlPosts(String username) {
		...
		String sqlQuery = "DELETE FROM posts WHERE P_UID= ?";
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery);
			preparedStatement.setString(2, username);
			preparedStatement.execute();
			
			return checkForAllPosts(username);
		} catch(SQLException throwables) {
			throwables.printStackTrace();
			return false;
		}
